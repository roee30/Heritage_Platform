...
let get_sandhi flag_pv =
  ...
  | c :: _ ->
      if c < 0
      then failwith "get_sandhi 5"
      else
        if c > 49
        then failwith "get_sandhi 6"
        else
          (let sandhis = sandhio.(c)
           in if flag_pv then filter_cesura sandhis else sandhis)
...
type rewrite_set = rule Deco.deco

let project n =
  function
  | Deco ((_, arcs)) -> (try List.assoc n arcs with | Not_found -> empty)
and get_rules = function | Deco ((rules, _)) -> rules
  
(* Union of two decos *)
let rec merger d1 d2 =
  match d1 with
  | Deco ((i1, l1)) ->
      (match d2 with
       | Deco ((i2, l2)) ->
...
module Auto = Share.Share(struct type domain = auto
                                  let size = hash_max
                                     end)
...
and hash b arcs rules =
  (* NB. [abs] needed because possible integer overflow *)
  (abs ((arcs + (Gen.dirac b)) + (List.length rules))) mod hash_max
  
(* [flag_pv=True] for preverbs automaton *)
let build_auto flag_pv (rewrite : rewrite_set) =
  let rec
    ...
    | Trie.Trie ((b, arcs)) ->
        ...
        let f (deter, stack, rewrite, span) (n, t) =
          ...
          let (auto, st, rew, k) = traverse current t
          in
        ...
        let s =
          Auto.share
            (State ((b, (List.rev deter), ((get_rules rewrite) @ h)))) key
        in (s, (merge local_stack l), rewrite, key)
  in traverse
...
let print_sandhi ps (w, u, v) =
  (ps (rdecode u); ps "|"; ps (decode v); ps " -> "; ps (decode w); ps "\n")
  
let print_cont_sandhi ps word rule =
  (ps ("[" ^ ((rdecode word) ^ "]")); print_sandhi ps rule)
  
let record degree assoc = (* [assoc] is a list [[ (deg1,count1); ... ]] *)
  let (left, right) = List2.zip degree assoc in
  let update =
    match right with
    | [] -> (degree, 1) :: right
    | (d, n) :: rest ->
        if d = degree then (d, (n + 1)) :: rest else (degree, 1) :: right
  in List2.unstack left update
  
let inspect_bool states =
  let count (t, f) =
    function | State ((b, _, _)) -> if b then ((t + 1), f) else (t, (f + 1))
  in List.fold_left count (0, 0) states
  
let add2 (sumt, sumf) states =
  let (t, f) = inspect_bool states in ((sumt + t), (sumf + f))
  
let inspect_choices spans states =
  let degrees sp =
    function | State ((_, _, choices)) -> record (List.length choices) sp
  in List.fold_left degrees spans states
  
(* Print automaton statistics in file [automaton.txt]. *)
let print_stats ps memo =
  let pi n = ps (string_of_int n) in
  let (tt, tf) = Array.fold_left add2 (0, 0) memo
  and spans = Array.fold_left inspect_choices [] memo
  and print_spans =
    let psp (sp, count) = (pi count; ps " states of degree "; pi sp; ps "\n")
    in List.iter psp
  and print_sandhis = List.iter (fun l -> List.iter (print_sandhi ps) l)
  and print_cont_sandhis cont =
    function
    | [ lev1; lev2 ] ->
    ...
    | [] -> ()
    | _ -> failwith "ill-formed sandhi table" in
  let print_sandhi_rules () =
    let print_table tab = for i = 1 to 49 do print_sandhis tab.(i) done
    and print_table_in_context tab =
      for i = 1 to 49 do print_cont_sandhis [ i ] tab.(i) done
    in
      (print_table sandhio;
       print_table_in_context sandhir;
       print_table_in_context sandhis)
  in
    (ps "total number of states: ";
     pi (tt + tf);
     ps " of which ";
     pi tt;
     ps " accepting\n";
...
let compile stats_flag rewrite lexicon =
  let (transducer, stack, _, _) = build_auto false rewrite [] lexicon
  in
    match stack with
    | [] ->
        (if stats_flag
         then (* optional monitoring *)
           (let cho = open_out Data.automaton_stats in
            let ps = output_string cho
            in (print_stats ps Auto.memo; close_out cho))
         else ();
         transducer)
    | _ ->
...
let compile_pv stats_flag rewrite lexicon =
  let (transducer, stack, _, _) = build_auto true rewrite [] lexicon
  in
    match stack with
    | [] ->
        (if stats_flag
...
let rec visual =
  ...
  | State ((b, deter, choices)) ->
      (print_string "<State";
       ...
       print_string " >")
and print_det (n, state) = (print_int n; visual state)
