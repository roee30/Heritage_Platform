...
let (sandhis, sandhir, sandhin, sandhif, sandhio) =
  (Gen.gobble Data.public_sandhis_file :
...
let get_sandhi =
  ...
  | c :: _ ->
      ...
      else if c > 49 then failwith "get_sandhi 6" else sandhio.(c)
...
let build_auto (rewrite : rewrite_set) =
  let rec
...
let compile rewrite lexicon =
  let (transducer, stack, _, _) = build_auto rewrite [] lexicon
  in
    match stack with
    | [] -> transducer
    | _ ->
