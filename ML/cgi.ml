...
let decode_url s =
  let rec need_decode i =
    ...
    then (match s.[i] with | '%' | '+' -> true | _ -> need_decode (succ i))
    else false in
    ...
    then
      (let i =
         ...
         | '%' when (i + 2) < (String.length s) ->
             ...
             in (Bytes.set s1 i1 (Char.chr v); i + 3)
         | '+' -> (Bytes.set s1 i1 ' '; succ i)
         | x -> (Bytes.set s1 i1 x; succ i)
       in copy_decode_in s1 i (succ i1))
    else s1 in
...
let create_env s =
  let rec get_assoc beg i =
    if i == (String.length s)
    then if i == beg then [] else [ String.sub s beg (i - beg) ]
    else
      if (s.[i] == ';') || (s.[i] == '&')
      then
        (let next_i = succ i
         in (String.sub s beg (i - beg)) :: (get_assoc next_i next_i))
  ...
  in List.map (separate 0) (get_assoc 0 0)
  
(* ddr end *)
let get key alist default =
  try List.assoc key alist with | Not_found -> default
  
let decoded_get key default alist = decode_url (get key alist default)
  
let query_string_env_var = "QUERY_STRING"
  
let query_string () =
  (* query is retrieved in system env var [QUERY_STRING] *)
  ...
  let url_encode =
    ...
    | (* Unreserved characters *)
        ('a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '-' | '.' | '_' | '~' | '|' |
         ...
         as c)
        -> String.make 1 c
    | (* NEW for corpus *) (* Special case of the space character *) ' ' ->
        "+"
    | (* Reserved characters *) c -> "%" ^ (hexa_str c) in
  let char_of_string s =
    if (String.length s) = 1 then s.[0] else failwith "char_of_string" in
  let subst s =
    ((s |> Str.matched_string) |> char_of_string) |> url_encode in
  let any_char = Str.regexp ".\\|\n"
  in Str.global_substitute any_char subst s
  
let query_of_env env =
  String.concat "&" (List.map (fun (k, v) -> k ^ ("=" ^ (url_encode v))) env)
  
let url ?query ?fragment path =
  let opt_part prefix = function | None -> "" | Some part -> prefix ^ part in
  let query_part = opt_part "?" query in
  let fragment_part = opt_part "#" fragment
  in path ^ (query_part ^ fragment_part)
