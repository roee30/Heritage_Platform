...
let person_of_subst =
  function | "aham" -> First | "tvad" -> Second | _ -> Third
  
let gram_role num entry =
  function
  | Nom -> Subject ((person_of_subst entry), num)
  | Acc -> Object
  | (* Patient or adverb of manner *) Ins -> Instrument
  | (* Agent or adverb of instrument *) Dat -> Destination
  | Abl -> Origin
  | Gen -> Possessor
  | Loc -> Circumstance
...
and case_of =
  function
  | (* inverse of [gram_role] *) Subject (_, _) -> Nom
  | Object -> Acc
  | Instrument -> Ins
  | Destination -> Dat
  | Origin -> Abl
...
let ini_mood = (0, 0)
and add_mood m moods =
  match m with
  | Imper b ->
      let (imp, inj) = moods
      in if b then ((imp + 1), inj) else (imp, (inj + 1))
  | _ -> moods
...
let root_regime =
  ...
  | (* akarmaka roots, checked by Pawan Goyal *)
      ...
      "an#2" | "as#1" | "as#2" | "aas#2" | "iih" | "uc" | "uurj#1" | ".rdh" |
        "edh" | "kamp" | "kaaz" | "kuc" | "ku.t" | "kup" | "kul" | "kuuj" |
        "k.lp" | "krii.d" | "krudh#1" | "klid" | "kvath" | "k.sar" | "k.si" |
        "k.su" | "k.sudh#1" | "k.subh" | "khel" | "gaj" | "garj" | "gard" |
        "galbh" | "gu~nj" | "gur" | "g.rr#2" | "glai" | "gha.t" | "gha.t.t" |
        "ghuur.n" | "cakaas" | "ca~nc" | "cal" | "cit#1" | "ce.s.t" | "jan" |
        "jaag.r" | "jiiv" | "j.rmbh" | "j.rr" | "jyaa#1" | "jvar" | "jval" |
        ".dii" | "tan#2" | "tam" | "tu.s" | "t.r.s#1" | "trap" | "tras" |
        "tvar" | "tsar" | "dak.s" | "dal" | "das" | "dah#1" | "dih" |
        "diik.s" | "diip" | "du.s" | "d.rh" | "dev#1" | "dyut#1" | "draa#1" |
        "draa#2" | "dhaav#1" | "dhru" | "dhvan" | "dhv.r" | "na.t" | "nand" |
        "nard" | "naz#1" | "nah" | "nii#1" | "n.rt" | "pat#1" | "pii" |
        "puuy" | "p.r#2" | "pyaa" | "prath" | "phal" | "ba.mh" | "bal" |
        "bha.n.d" | "bhand" | "bha.s" | "bhaa#1" | "bhaas#1" | "bhii#1" |
        "bhuj#1" | "bhuu#1" | "bhra.mz" | "bhram" | "bhraaj" | "ma.mh" |
        "majj" | "mad#1" | "mud#1" | "muh" | "muurch" | "m.r" | "m.rdh" |
        "mre.d" | "mlaa" | "yabh" | "yas" | "yu#2" | "yudh#1" | "ra~nj" |
        "ra.n" | "ram" | "raaj#1" | "ru" | "ruc#1" | "rud#1" | "ru.s#1" |
        "ruh#1" | "lag" | "lamb" | "lal" | "las" | "vak.s" | "vas#1" |
        "vah#1" | (* nadii vahati *) "vaa#2" | "vaaz" | "vij" | "vip" |
        "viz#1" | "v.rt#1" | "v.rdh#1" | "vyath" | "zak" | "zad" | "zam#1" |
        "zii#1" | "ziil" | "zuc#1" | "zudh" | "zubh#1" | "zu.s" | "zuu" |
        "zram" | "zrambh" | "zvas#1" | "zvit#1" | "sap#1" | "saa#1" |
        "sidh#1" | "sur" | "skhal" | "stan" | "stu" | "stubh" | "sthaa#1" |
        "snih#1" | "snu" | "spand" | "spardh" | "sphaa" | "sphu.t" | "sphur"
        | "smi" | "syand" | "sra.ms" | "svap" | "svar#1" | "svar#2" | "had" |
        "has" | "hikk" | "h.r.s" | "hras" | "hraad" | "hrii#1" | "hlaad" |
        "hval"
      -> Intransitive
...
let root_regime_gana k =
  ...
  | "tap" | "pac" | "raadh" | "svid#2" ->
      (match k with | 4 -> Intransitive | _ -> Transitive)
...
*)
let agent_of_passive =
  function
  | "vid#2" -> []
  | (* ellipsed impersonal agent "it is known that" *) _ -> [ Instrument ]
  
(* Agent at instrumental in passive voice *)
...
let regime entry (cj, t) =
  (* conjugation cj and possible preverb sequence ignored in first version *)
  let regime = root_regime entry
  in
    (* TODO dependency on k *)
    match t with
    | Conjug (_, Passive) | Presentp _ ->
        let aspect =
          (match regime with
           ...
           | Factitive -> Statif
           | _ -> Perfectif)
        and valency = agent_of_passive entry
        in (aspect, valency, Indicative)
    | Conjug (t, _) ->
        let aspect =
          if (regime = Factitive) || (regime = Quotative)
          then Statif
          else Imperfectif
        and valency =
          (match regime with | Transitive -> [ Object ] | _ -> [])
        and mood =
          (match t with | Injunctive _ -> Imper false | _ -> Indicative)
        in (aspect, valency, mood)
    | Presenta (k, m) | Presentm (k, m) ->
        (* on affine le regime gana et mode *)
        let regime = root_regime_gana k entry in
        let aspect =
          if (regime = Factitive) || (regime = Quotative)
          then Statif
          else
            (* if m=Optative then Statif (* NEW bruyaat *)
                     else *)
            Imperfectif
        and valency =
          (match regime with | Transitive -> [ Object ] | _ -> [])
        and mood =
          (match m with | Imperative -> Imper true | _ -> Indicative)
        in (* now, only Imperative for Present *) (aspect, valency, mood)
    | Perfut _ ->
        ((if regime = Factitive then Statif else Imperfectif),
         (match regime with | Transitive -> [ Object ] | _ -> []),
         Indicative)
  
let get_fin_roles entry f n p =
  let (aspect, valency, mood) = regime entry f in
  let demand =
    match aspect with
    | Statif | Impersonal -> valency
    | _ -> (Subject (p, n)) :: valency
  in
    (* anaphoric subject reference *)
...
let is_ppra (_, v) = match v with | (* TEMP *) Ppra _ -> true | _ -> false
...
let get_roles entry =
  ...
  | Part_form (v, g, n, c) ->
      ...
      then Addressee
        ...
        if is_ppra v
        then Subprocess []
        else (* should lookup root *) Actor ((gram_role n entry c), g, n)
  | (* beware n duplication *) Noun_form (g, n, c) ->
      if c = Voc
      then Addressee
      else
        if (g = (Deictic Numeral)) || (entry = "eka")
        then Number (g, c, n)
        else Actor ((gram_role n entry c), g, n)
  | (* beware n duplication *) Verb_form (f, n, p) ->
      get_fin_roles entry f n p
  | Abs_root _ -> get_abs_roles entry
  | Ind_form Conj ->
      (match entry with | "ca" -> Tool Coordination | _ -> Ignored)
  | (* TODO vaa etc *) Ind_form Prep ->
      if (entry = "saha") || ((entry = "vinaa") || (entry = "satraa"))
      then Tool Post_instrument
      else Ignored
  | Ind_form Adv ->
      if entry = "saha" then Tool Not_Post_instrument else Ignored
  | Ind_form Abs -> get_abs_roles entry
  | Ind_form Part ->
      (match entry with | "maa#2" -> Tool Prohibition | _ -> Ignored)
  | Bare_stem -> Compound
  | _ -> Ignored
  
(* Used in Parser, Reader *)
let roles_of seg word tags =
  let distrib (sub, res) (delta, morphs) =
    ...
    let (_, r) =
      let label (i, l) role = ((i + 1), (((seg, sub, i), role) :: l))
      in List.fold_left label (1, res) roles
    in ((sub + 1), r) in
  let (_, rls) = List.fold_left distrib (1, []) tags in rls
...
type label =
  (int * int * int)
  and (* (segment number, homonymy index, tag index) *)
  roles =
  (label * pos) list
...
let rec flatten_add =
  function
  | (* arg goes backward in time *) [] -> [ [] ]
  | l :: r -> (* l: roles *)
      let flatr = flatten_add r
      and distr res f = (* f: roles *)
        let prefix acc x = (x :: f) :: acc in
        let result = List.fold_left prefix [] l in result @ res
      in List.fold_left distr [] flatr
...
let merge =
  function
  | (* persons priorities *) First -> (fun _ -> First)
  | Second -> (function | First -> First | _ -> Second)
  | Third -> (function | First -> First | Second -> Second | _ -> Third)
and add =
  function
  | (* numbers additions *) Plural | Dual -> (fun _ -> Plural)
  | Singular -> (function | Plural | Dual -> Plural | Singular -> Dual)
  
let rec dom =
  function
  | (* male dominance *) Mas -> (fun _ -> Mas)
  | Fem ->
      (function | Mas -> Mas | Deictic d -> dom Fem (context d) | _ -> Fem)
  | Neu -> (function | Deictic d -> context d | g -> g)
...
let end_coord kar acc p g n =
  let rec rem_iic cur =
    match cur with
    | Compound :: rest -> rem_iic rest
    | _ ->
        (match kar with
         | (* Synthesis of compound kar *) Subject (_, _) ->
             (((Actor ((Subject (p, n)), g, n)) :: acc), cur)
         | kar -> (((Actor (kar, g, n)) :: acc), cur))
  in rem_iic
  
let agree_deictic g = function | Deictic _ -> true | g1 -> g = g1
  
(* Remove compound formation and possible adjectival number word. *)
let skim c g n context =
  let rec skim_rec con =
    match con with
    | Compound :: rest -> skim_rec rest
    | (* skip possible iic - compounding *) Number (g1, c1, n1) :: rest ->
        if (agree_deictic g g1) && ((c = c1) && (n = n1))
        then (* agreement of Number *) rest
        else raise No_coord
    | _ -> con
  in skim_rec context
  
let rec coord1 kar acc p g n =
  function
  | (* searching for closest noun phrase *) [] -> raise No_coord
  | np :: rest ->
      (match np with
       | Actor ((Subject (p1, _)), g1, n1) ->
           (match kar with
            | Subject (_, _) ->
       ...
       | _ -> raise No_coord)
and coord2 kar acc p g n cur =
  match cur with
  | (* searching for previous noun phrases *) [] -> raise No_coord
  | np :: rest ->
      (match np with
       | Actor ((Subject (p1, _)), g1, n1) ->
           (match kar with
            | Subject (_, _) ->
                let before = skim Nom g1 n1 rest
                in
       ...
       | Actor (k, g1, n1) ->
           if k = kar
           then
             (let before = skim (case_of k) g1 n1 rest
              in
                (* additive interpretation of ca *)
                end_coord kar acc Third (dom g g1) (add n n1) before)
           else raise No_coord
       | Tool Coordination -> coord1 kar acc p g n rest
       | (* iterate the tool *) _ -> raise No_coord)
  
(* Coordination: the ca tool constructs a composite tag from its predecessors *)
let coordinate acc =
  function
  | (* searching for first noun phrase *) [] -> raise No_coord
  | np :: rest ->
      (match np with
       | Actor (((Subject (p1, _) as kar)), g1, n1) ->
           let before = skim Nom g1 n1 rest in coord2 kar acc p1 g1 n1 before
       | Actor (kar, g1, n1) ->
           let before = skim Nom g1 n1 rest
           in coord2 kar acc Third g1 n1 before
       | _ -> raise No_coord)
  
(* Bumping the current penalty by a given malus *)
let penalize malus (roles, pen) = (roles, (pen + malus))
  
(* Ugly experimental management of "maa" negative particle - temporary *)
let maa_counter = ref 0
  
let reset_maa () = maa_counter := 0
  ...
  | r :: iroles ->
      (match r with
       | Tool Coordination -> (* ca *)
           (try
              let (oroles, penalty) = res in
              let (result, left) = coordinate oroles iroles
              in use_tools (result, penalty) left
            with | No_coord -> use_tools (penalize coord_penalty res) iroles)
       | Tool Post_instrument -> (* saha vinaa prep *)
           (match iroles with
            | [] -> penalize 1 res
            | r :: previous ->
                (match r with
                 | Actor (Instrument, _, _) -> use_tools res previous
                 | (* <i.-saha> *) _ -> use_tools (penalize 1 res) iroles))
       | Tool Not_Post_instrument -> (* saha adv *)
           (match iroles with
            | [] -> res
            | r :: _ ->
                (match r with
                 | Actor (Instrument, _, _) ->
                     use_tools (penalize 1 res) iroles
                 | _ -> use_tools res iroles))
       | Tool Prohibition -> (* maa *) (maa_counter := !maa_counter + 1; res)
       | Tool _ | (* not yet implemented *) Ignored | (* noop *) Compound ->
           use_tools res iroles
       | (* compounds are skipped *)
           (* ordinary roles are processed as Identity tools *) _ ->
           let (oroles, p) = res
           in
             (* otherwise we take role as is *)
...
let process_role (neg, pos, pro, subpro, moo) role =
  match role with
  | Process (noun_roles, m) ->
      ((noun_roles @ neg), pos, (pro + 1), subpro, (add_mood m moo))
...
let subject_agreement (noun_role, _, _) p n = noun_role = (Subject (p, n))
...
let remove_subj p n =
  let rec remrec acc =
    ...
    | [] -> raise Missing
    | triple :: rest ->
        if subject_agreement triple p n
        then (triple, (List2.unstack acc rest))
        else remrec (triple :: acc) rest
  in remrec []
  
(* Tries to find a matching role for a non-agent [noun_role] *)
let remove_matching kar =
  let rec remrec acc =
    function
    | [] -> raise Missing
    | (((k, _, _) as triple)) :: rest ->
        if k = kar
        then (triple, (List2.unstack acc rest))
        else (* we choose latest matching *) remrec (triple :: acc) rest
  in remrec []
...
let process_exp (missing, taken, left) =
  function
  | (* for each expected [noun_role] we look for a matching given one *)
      Subject (p, n) -> (* verb subject has p and n *)
      (try
         let (found, remain) = remove_subj p n left
         in (missing, (found :: taken), remain)
       with | Missing -> (missing, taken, left))
  | (* subject is optional *) kar ->
      (try
         let (found, remain) = remove_matching kar left
         in (missing, (found :: taken), remain)
       with | Missing -> ((kar :: missing), taken, left))
...
let contract taken =
  let filter left triple =
    if List.mem triple taken then left else triple :: left
  in List.fold_left filter []
  
(* Penalty parameters in need of tuning by training *)
let missing_role_penalty _ = 1
and excess_subject_penalty = 1
and np_penalty = 2
and absol_penalty = 2
  
(* absolutive without finite verb *)
(* remaining extra nominatives give penalty *)
let count_excess pen =
  function
  | (Subject (p, n), _, _) -> pen + excess_subject_penalty
  | triple -> pen
  
(* taken as adverbs or genitive noun phrases *)
(* We count all persons with same person and number *)
let count_subj persons =
  function | Subject (p, n) -> List2.union1 (p, n) persons | _ -> persons
  
let count_missing pen k = pen + (missing_role_penalty k)
  
let missing_penalty = List.fold_left count_missing 0
...
let rec show_penalty =
  function
  | (* explicit vector for debug *) Sentence ((p1, p2, p3, p4)) ->
      "S(" ^
        ((string_of_int p1) ^
           ("," ^
              ((string_of_int p2) ^
...
let rec eval_penalty =
  function
  | Sentence ((pen1, pen2, pen3, pen4)) -> ((pen1 + pen2) + pen3) + pen4
  | Copula ((pen1, pen2, pen3, pen4, pen5)) ->
      (((pen1 + pen2) + pen3) + pen4) + pen5
  | NP pen -> np_penalty + (eval_penalty pen)
  
let balance_process pro subpro =
  if pro > 1
  then pro - 1
  else (* TEMP, to be adjusted with relative clauses *)
    if pro = 0 then if subpro then absol_penalty else 0 else 0
  
(* Delay dealing with nominatives in order to favor Acc over Nom for neuters *)
let sort_kar =
  let rec sort_rec nomins others n =
    function
...
let check_sentence pen1 neg pos pro subpro =
  let (missing, taken, left) =
    List.fold_left process_exp ([], [], pos) neg in
  let contracted = contract taken left in
  let pen2 = missing_penalty missing
  and pen3 = excess_penalty contracted
  and pen4 = balance_process pro subpro
  in Sentence ((pen1, pen2, pen3, pen4))
...
let process_exp_g p n roles =
  let remove_matching =
    let rec remrec acc =
      ...
      | triple :: rest ->
          (match triple with
           | (Subject (p, n'), _, _) when n' = n ->
               (triple, (List2.unstack acc rest))
           | (* NB there is no mandatory concord of genders *) _ ->
               remrec (triple :: acc) rest)
    in remrec []
  in
    (* we look for a matching nominative *)
    try
      let (found, remain) = remove_matching roles in ([], [ found ], remain)
    with
    | Missing -> (* First and Second persons Subjects are optional *)
        if (p = First) || (p = Second)
        then ([], [], roles)
        else ([ Subject (p, n) ], [], roles)
  
let check_copula_sentence pen1 p n pos subpro =
  let (missing, taken, left) = process_exp_g p n pos in
  let contracted = contract taken left in
  let pen2 = missing_penalty missing
  and pen3 = excess_penalty contracted
  and pen4 = if subpro then absol_penalty else 0
  in Copula ((pen1, pen2, pen3, pen4, 0))
  
(* [get_predicate] returns the first available Subject (backward from the end)
   if there is one, else raises Missing *)
let get_predicate =
  let rec search_subject acc =
    function
    | [] -> raise Missing
    | (((kar, _, _) as triple)) :: rest ->
        (match kar with
...
let rec mood_correction (imp, inj) pen =
  let maa_tokens = !maa_counter in (* counted by Prohibition tool *)
  let maa_pen =
    if maa_tokens > (imp + inj)
    then maa_tokens - (imp + inj)
    else if inj > maa_tokens then inj - maa_tokens else 0
  in
    match pen with
    | Sentence ((p1, p2, p3, p4)) -> Sentence ((p1, p2, p3, (p4 + maa_pen)))
    | (* p4=0 *) Copula ((p1, p2, p3, p4, p5)) ->
        Copula ((p1, p2, p3, p4, (p5 + maa_pen)))
    | (* p5=0 *) NP pen -> mood_correction (imp, inj) pen
  
(* weird *)
let inspect pen (neg, pos, pro, subpro, md) =
  let pens =
    if neg = []
    then (* no overt verb, we conjecture copula (pro=0) *)
      (try
         let (p, n, rest) = get_predicate pos
         in check_copula_sentence pen p n rest subpro
       with
       | Missing -> (* maybe noun phrase *)
           NP (check_sentence pen [] pos pro subpro))
    else (* 2+ *) check_sentence pen neg pos pro subpro
  in mood_correction md pens
  
(* verbal predicate exists *)
...
let penalty rev_path =
  let right_left_roles = List.map snd rev_path
  in
    (reset_maa ();
     (* horreur *)
     let (roles, pen_tools) = use_tools ([], 0) right_left_roles in
     let constraints =
       List.fold_left process_role ([], [], 0, false, ini_mood) roles
     in inspect pen_tools constraints)
  
type flattening = (penalty * (roles list)) list

(* We flatten all choices in the chunked solution *)
(* [sort_flatten: list roles -> flattening] *)
let sort_flatten groups = (* groups goes backward in time *)
  ...
  let insert_in sorted_buckets rev_path =
    ...
    let rec ins_rec acc =
      ...
      | ((((pk, b_k) as b)) :: r as buckets) ->
          let ek = eval_penalty pk
          in
            (* recomputation to avoid *)
            if ek = ep
            then List2.unstack acc ((p, (rev_path :: b_k)) :: r)
            else
              if ek < ep
              then ins_rec (b :: acc) r
              else List2.unstack acc ((p, [ rev_path ]) :: buckets)
    in ins_rec [] sorted_buckets in
  let sort_penalty = List.fold_left insert_in [] in sort_penalty parses
  
(* Output truncated to avoid choking on immense web page.
   Returns penalty threshold if truncation. Used in Reader and Parser *)
let truncate_groups buckets =
  match buckets with
  | best :: next :: rest ->
      let top = [ best; next ] in (* top 2 buckets *)
      let threshold =
        (match rest with | [] -> None | (p, _) :: _ -> Some (eval_penalty p))
      in (top, threshold)
  | _ -> (buckets, None)
  
let extract str ((seg, sub, ind), _) = (* construct tag projections *)
  let m = string_of_int sub
  and (* segment number [seg] is redundant *) n = string_of_int ind in
  let proj = m ^ ("," ^ n) in if str = "" then proj else proj ^ ("|" ^ str)
