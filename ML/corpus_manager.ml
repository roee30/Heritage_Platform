...
(* The following functions assume that the given list is sorted in
   ...
   integers.  *)
let max_gap = { start = 1; stop = max_int; }
  
let string_of_gap gap =
  if gap.stop = max_int
  then Printf.sprintf "> %d" (gap.start - 1)
  else Printf.sprintf "%d - %d" gap.start gap.stop
...
let rec first_group =
  ...
  | x :: ((y :: _ as t)) ->
      if y = (x + 1)
      then
        (let (group, gap, rest) = first_group t in ((x :: group), gap, rest))
      else ([ x ], { start = x + 1; stop = y - 1; }, t)
  | [] -> ([], max_gap, [])
  | ([ x ] as l) -> (l, { start = x + 1; stop = max_int; }, [])
  
let groups_with_gaps l =
  let rec aux l =
    let (group, gap, rest) = first_group l in
    let group_gap = (group, gap)
    in match rest with | [] -> [ group_gap ] | _ -> group_gap :: (aux rest)
  in aux l
  
let add_init_gap groups =
  let init_gap =
    function
    | (x :: _, _) :: _ ->
...
(* Page generation *)
(*******************)
let big text = div Latin16 text
  
let link permission dir =
  let url =
    let query =
      Cgi.query_of_env
        [ (Params.corpus_dir, dir);
          (Params.corpus_permission,
           (Web_corpus.string_of_permission permission)) ]
    in (Cgi.url corpus_manager_cgi ~query) |> escape in
  let label = Filename.basename dir in anchor_ref url label
  
let uplinks dir permission =
  let aux dir =
    ...
    let updirs =
      List.mapi
        ...
        updirs
    in List.map (link permission) updirs in
  let uplinks_str = (dir |> aux) |> (String.concat " / ") in
  let final_sep = if uplinks_str <> "" then " / " else ""
  in uplinks_str ^ final_sep
  
(* Display sentences with format "sentence || sentno" like in citations
   file.  *)
let sentence_links dir permission sentences =
  let to_anchor_ref sentence =
    let font = font_of_string Paths.default_display_font in
    let encoding =
      match font with
      | Deva -> Corpus.Encoding.Devanagari
      | Roma -> Corpus.Encoding.IAST in
    let text = Corpus.Sentence.text encoding sentence in
    let display =
      match font with | Deva -> deva16_blue | Roma -> span Trans16
    in
      (text |>
         (anchor_ref
            ((sentence |> (Web_corpus.url dir permission)) |> escape)))
        |> display
  in List.map to_anchor_ref sentences
  
let section_selection dir sections =
  let options =
    let prefixes = List.map (fun x -> Filename.concat dir x) sections
    in List.combine prefixes sections
  in option_select_label Params.corpus_dir options
  
let add_sentence_form dir permission gap =
  (cgi_begin (cgi_bin "skt_heritage") "") ^
    ("Add sentence: " ^
       ((uplinks dir permission) ^
          ((hidden_input Params.corpus_dir dir) ^
             ((hidden_input Params.corpus_permission
                 (Web_corpus.string_of_permission permission))
                ^
...
let htmlify_group dir permission (group, gap) =
  let (ol, group_id) =
    ...
    | h :: _ ->
        ...
        let group_id = string_of_int id
        in
          ((ol ~li_id_prefix: "" ~start: id
              (sentence_links dir permission group)),
           group_id) in
  let div_id = "group" ^ group_id in
  let add_sentence_form =
    (button ~id: "add_sentence"
       ~onclick: { js_funid = "hideShowElement"; js_funargs = [ div_id ]; }
       (string_of_gap gap))
      ^
      ((elt_begin_attrs [ ("id", div_id) ] "div" Hidden_) ^
         (html_paragraph ^ ((add_sentence_form dir permission gap) ^ div_end)))
  in
    ol ^
      (if permission = Web_corpus.Annotator then add_sentence_form else "")
  
let group_sentences dir sentences =
  let ids = List.map Corpus.Sentence.id sentences in
  let dict = List.combine ids sentences in
  let groups = (ids |> groups_with_gaps) |> add_init_gap
  in
    List.map (fun (x, y) -> ((List.map (fun x -> List.assoc x dict) x), y))
      groups
  
let new_section_form dir permission =
  let perm = Web_corpus.string_of_permission permission
  in
...
let section_selection_form dir permission sections =
...
let body dir permission =
  ...
  | Web_corpus.Empty ->
      (((uplinks dir permission) |> big) |> pl;
       ...
       (match permission with
        ...
        | Web_corpus.Manager -> new_section_form dir permission) |> pl;
       close_page_with_margin ())
  | Web_corpus.Sentences sentences ->
      let groups = group_sentences dir sentences
      in
        (((uplinks dir permission) |> big) |> pl;
         open_page_with_margin 30;
         if permission = Web_corpus.Manager
         then "No action available." |> pl
         else
           (groups |> (List.map (htmlify_group dir permission))) |>
             (List.iter pl);
         close_page_with_margin ())
  | Web_corpus.Sections sections ->
      (center_begin |> pl;
       ...
       html_break |> pl;
       if permission = Web_corpus.Manager
       then (new_section_form dir permission) |> pl
       else ();
       center_end |> pl)
  
let capitalize_ascii str =
  (* Oct 2020 [String.capitalize_ascii] deprecated *)
  ((str |> Bytes.of_string) |> Bytes.capitalize_ascii) |> Bytes.to_string
  
let mk_page dir permission =
  let title_str =
    "Sanskrit Corpus " ^
      ((permission |> Web_corpus.string_of_permission) |> capitalize_ascii) in
  let clickable_title =
    let query =
      Cgi.query_of_env
        [ (Params.corpus_permission,
           (Web_corpus.string_of_permission permission)) ]
    in
      (title_str |> (anchor_ref (Cgi.url corpus_manager_cgi ~query))) |>
        h1_title
  in
    (maybe_http_header ();
