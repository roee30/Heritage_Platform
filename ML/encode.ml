...
(* [code_raw] and similar *)
open Phonetics
  
(* homonasal vowel *)
exception In_error of string
  
(* Error in user or corpus input *)
let is_vowel c = (vowel c) || ((avagraha c) || ((c > 100) && (c < 114)))
  
(* Deprecated accounts for upper case *)
(* anusvara substituted by nasal or normalized to 14 when original *)
(* anunaasika after vowels treated as anusvaara *)
let rec normalize : Word.word -> Word.word =
  let rec normal_rec after_vow =
    function
    | [] -> []
    | [ 14 ] -> (* .m *) [ 14 ]
    | (* and NOT m *) 14 :: (* .m *) c :: l ->
        if after_vow
        then (let c' = homonasal c in c' :: c :: (normal_rec (is_vowel c) l))
        else raise (In_error "Anusvaara should follow vowel")
    | 15 :: (* ~~ *) c :: l -> (* 31-08-19 anunaasika normalisation *)
        if after_vow
        then (* anunaasika assimilated to anasvaara *)
  ...
  | [ 16 (* .h *) :: [ c :: l ] ] -> 
   ...
   Apte avoids the problem by putting all dus-X under dus.  *)
        50 :: l -> 50 :: (normal_rec false l)
    | (* hiatus *) c :: l -> c :: (normal_rec (is_vowel c) l)
  in normal_rec false
  
let code_string str = normalize (code_raw str)
and (* standard VH *) code_string_WX str = normalize (code_raw_WX str)
...
and (* with upper letters *) code_skt_ref_d str = normalize (code_rawd str)
  
(* no diacritics *)
(* Switching code function according to transliteration convention *)
let switch_code : string -> string -> Word.word =
  function
  | (* normalizes anusvaara in its input *) "VH" -> code_string
  | (* [Canon.decode]    *) "WX" -> code_string_WX
  | (* [Canon.decode_WX] *) "KH" -> code_string_KH
  | (* [Canon.decode_KH] *) "SL" -> code_string_SL
  | (* [Canon.decode_SL] *) _ -> failwith "Unknown transliteration scheme"
  
let rev_code_string str = Word.mirror (code_string str)
  
(* [anchor : string -> string] -- used in [Morpho_html.url] and Sanskrit *)
let anchor t =
  let canon c =
    if c > 100 then Canon.canon_upper_html c else Canon.canon_html c in
  let catenate c (s, b) = (* similar to Canon.catenate *)
    let b' = (c > 0) && (c < 14) in (* Phonetics.vowel c *)
    let hiatus = if b && b' then "_" ^ s else s in (((canon c) ^ hiatus), b') in
...
let strip w =
  match w with
  | last :: rest -> if last > 50 then rest else (* remove homonymy index *) w
  | [] -> failwith "Empty stem to strip"
  
let rstem w = strip (Word.mirror w)
  
let rev_strip w = Word.mirror (rstem w)
  
(* [compute_mw_links] *)
(* Builds revword normalised stem from entry string of root *)
(* Used by [Verbs.revstem], [Nouns.enter_iic], [Print_dict] *)
let rev_stem str = strip (rev_code_string str)
...
let decompose_str str = decompose (rev_code_string str)
  
(* ugly multiple reversals *)
let normal_stem str = Word.mirror (rev_stem str)
  
let normal_stem_str str = Canon.decode (normal_stem str)
...
let code_strip_raw s = rev_strip (code_raw s)
  
(* A cleaner solution to name spaces would be to have type lexeme = (word * int) 
   and "x#5" represented as (x,5) (0 if no homophone) *)
let skt_to_deva str =
  try Canon.unidevcode (code_string str) with | Failure _ -> failwith str
and skt_raw_to_deva str =
  try Canon.unidevcode (code_raw str)
  with | Failure _ -> failwith ("raw " ^ str)
and skt_strip_to_deva str =
  try Canon.unidevcode (code_strip_raw str)
  with | Failure _ -> failwith ("raw stripped " ^ str)
...
let diff_str str w = Word.diff w (code_string str)
  
(*i end; i*)
let devanagari_to_velthuis (s : string) : string = let open Uchar
  in
    let buf = Buffer.create (String.length s) in (* Tables *)
    let consonant_map = Hashtbl.create 100 and vowel_map = Hashtbl.create 20
    and matra_map = Hashtbl.create 20 and special_map = Hashtbl.create 10 in
    let add tbl cp str = Hashtbl.add tbl (Uchar.of_int cp) str
    in
      (List.iter (fun (cp, s) -> add vowel_map cp s)
       ...
       List.iter (fun (cp, s) -> add consonant_map cp s)
         [ (0x0915, "k"); (0x0916, "kh"); (0x0917, "g"); (0x0918, "gh");
           (0x0919, "\"n"); (0x091A, "c"); (0x091B, "ch"); (0x091C, "j");
           (0x091D, "jh"); (0x091E, "~n"); (0x091F, ".t"); (0x0920, ".th");
           (0x0921, ".d"); (0x0922, ".dh"); (0x0923, ".n"); (0x0924, "t");
           (0x0925, "th"); (0x0926, "d"); (0x0927, "dh"); (0x0928, "n");
           (0x092A, "p"); (0x092B, "ph"); (0x092C, "b"); (0x092D, "bh");
           (0x092E, "m"); (0x092F, "y"); (0x0930, "r"); (0x0932, "l");
           (0x0935, "v"); (0x0936, "\"s"); (0x0937, ".s"); (0x0938, "s");
           (0x0939, "h") ];
       List.iter (fun (cp, s) -> add matra_map cp s)
         [ (0x093E, "aa"); (0x093F, "i"); (0x0940, "ii"); (0x0941, "u");
           (0x0942, "uu"); (0x0943, ".r"); (0x0944, ".rr"); (0x0962, ".l");
           (0x0963, ".ll"); (0x0947, "e"); (0x0948, "ai"); (0x094B, "o");
           (0x094C, "au") ];
       ...
       (let decode = Uutf.decoder ~encoding: `UTF_8 (`String s) in
        ...
        let flush_pending pending next =
          match pending with
          | Some c ->
              if next = (Some virama)
          ...
          | None -> () in
        let rec loop pending next =
          match Uutf.decode decode with
          | `Uchar u ->
              ...
              then
                ...
                if Hashtbl.mem consonant_map u
                then
                  (flush_pending pending None;
                   loop (Some (Hashtbl.find consonant_map u)) (Some u))
                else
                  if Hashtbl.mem matra_map u
                  then
                    (match pending with
                     | Some cons ->
                         (Buffer.add_string buf
                            (cons ^ (Hashtbl.find matra_map u));
                          loop None None)
                     | None ->
                         (Buffer.add_string buf (Hashtbl.find matra_map u);
                          loop None None))
                  else
                    if Hashtbl.mem special_map u
                    then
                      (flush_pending pending None;
                      ...
                      if u = virama
                      then
                        (flush_pending pending (Some virama); loop None None)
                      else
                        (flush_pending pending None;
          ...
          | `Malformed _ ->
              (flush_pending pending None;
               Buffer.add_char buf '?';
               loop None None)
          | `End -> flush_pending pending None
        in loop None None);
       Buffer.contents buf)
