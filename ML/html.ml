...
let xml_begin xml_op = "<" ^ (xml_op ^ ">")
and xml_begin_with_att xml_op atts =
  "<" ^ (xml_op ^ ((quote_alist atts) ^ ">"))
and xml_end xml_op = "</" ^ (xml_op ^ ">")
and xml_empty xml_op = "<" ^ (xml_op ^ ">")
and xml_empty_with_att xml_op atts =
  "<" ^ (xml_op ^ ((quote_alist atts) ^ ">"))
  
let xml_next op = (xml_end op) ^ (xml_begin op)
  
let html_break = xml_empty "br"
and html_paragraph = (xml_begin "p") ^ (xml_end "p")
  
(* Array operations *)
let tr_begin = xml_begin "tr"
...
let text_area control width length sentence =
  let w = string_of_int width
  and l = string_of_int length
  in
    (xml_begin_with_att "textarea"
       [ ("id", input_id); ("name", control); ("rows", w); ("cols", l) ])
      ^
      (sentence ^
         ((xml_end "textarea") ^
            (* Caution - necessary to separate begin and end *)
            ("\n" ^
               ((xml_begin_with_att "script" [ ("type", "text/javascript") ])
                  ^ (focus_script ^ (xml_end "script"))))))
  
(* printing options for the user to choose lemma *)
let option_print id control =
  (xml_begin_with_att "option" [ ("value", id) ]) ^
    (control ^ (xml_end "option"))
  
let option_print_default id control b =
  let value_param = ("value", id) in
  let params =
    if b then [ value_param; ("selected", "selected") ] else [ value_param ] in
  let menu = xml_begin_with_att "option" params
  in menu ^ (control ^ (xml_end "option"))
...
let rec print_options_default =
  function
  | [] -> ""
  | (control, id, b) :: rest ->
      (option_print_default id control b) ^ (print_options_default rest)
  
let option_select list_options =
  (xml_begin "select") ^ ((print_options list_options) ^ (xml_end "select"))
  
let option_select_label label list_options =
  (xml_begin_with_att "select" [ ("name", label) ]) ^
    ((print_options list_options) ^ (xml_end "select"))
  
let option_select_default label list_options =
  (xml_begin_with_att "select" [ ("name", label) ]) ^
    ((print_options_default list_options) ^ (xml_end "select"))
  
let option_select_default_id id label list_options =
  (xml_begin_with_att "select" [ ("id", id); ("name", label) ]) ^
    ((print_options_default list_options) ^ (xml_end "select"))
  
let text_input id control =
  xml_empty_with_att "input"
    [ ("id", id); ("type", "text"); ("name", control) ]
  
let add_opt_attrs opt_attrs attrs =
  let add_attr acc (label, v) =
    match v with | None -> acc | Some v -> (label, v) :: acc
  in List.fold_left add_attr attrs opt_attrs
  
let int_input ?id ?value ?(step = 1) ?(min = min_int) ?(max = max_int) name =
  let attrs =
...
let radio_input control v label =
  let attrs = [ ("type", "radio"); ("name", control); ("value", v) ]
  in (xml_empty_with_att "input" attrs) ^ label
  
let select control = List.map (fun (label, v) -> radio_input control v label)
  
let radio_input_dft control v label checked =
  let check = if checked then [ ("checked", "checked") ] else [] in
  let attrs = [ ("type", "radio"); ("name", control); ("value", v) ] @ check
  in (xml_empty_with_att "input" attrs) ^ label
  
let select_default name =
  List.map (fun (label, v, checked) -> radio_input_dft name v label checked)
  
let submit_input label =
  xml_empty_with_att "input" [ ("type", "submit"); ("value", label) ]
  
let reset_input label =
  xml_empty_with_att "input" [ ("type", "reset"); ("value", label) ]
  
let hidden_input name label =
  xml_empty_with_att "input"
    [ ("type", "hidden"); ("name", name); ("value", label) ]
...
let li ?id item =
  let li = "li" in
  let attrs = add_opt_attrs [ ("id", id) ] []
  in (xml_begin_with_att li attrs) ^ (item ^ (xml_end li))
  
(* Ordered list *)
let ol ?id ?li_id_prefix ?(start = 1) items =
  let ol = "ol" in
  let process i item =
    let id =
      let genid prefix = prefix ^ (string_of_int (start + i))
      in Gen.opt_app genid li_id_prefix
    in li ?id item in
...
let fieldn name content = [ ("name", name); ("content", content) ]
...
let rgb =
  function
  | (* a few selected HTML colors in rgb data *)
      (* The colors of phases are defined in [Phases.color_of_phase] *) Black
      -> "#000000"
  | White -> "#FFFFFF"
  | (* Wheat = "#F0E0B0" ou "#F5DEB3" *) Red -> "#FF0000"
  | (* Firebrick = "#B02020" *) Blue -> "#0000FF"
  | (* Canard = "#0000C0" ou "#0080FF" *) Green -> "#008000"
  | (* Teal = "#008080" Olive = "#808000" *) Aquamarine -> "#6FFFC3"
  | (* actually Light Aquamarine *) Lawngreen -> "#66FF99"
  | (* was "#7CFC00" *) Yellow -> "#FFFF00"
  | Orange -> "#FFA000"
  | Cyan -> "#00FFFF"
  | (* Aqua = Cyan, Turquoise = "#40E0D0" *) Purple -> "#800080"
  | (* Plum = "#E0A0E0" *) Grey -> "#B8B8B8"
  | (* Slategrey = "#708090" *) Navy -> "#000080"
  | (* Midnight blue = "#101870" *) Deep_sky -> "#00C0FF"
  | Brown -> "#A02820"
  | (* Chocolate = "#D06820" *) Maroon -> "#800000"
  | Carmin -> "#FF1975"
  | (* Carmin = "#FF0066" Deep pink= "#FF1090" *) Chamois -> "#F5F5DC"
  | (* gris-beige *) Broon -> "#852B1D"
  | (* good with gold *) Gold -> "#A58959"
  | (* Silver = "#C0C0C0" *) Magenta -> "#FF00FF"
  | (* Violet = "#F080F0" Blueviolet = "#8028E0" *) Mauve -> "#FF99FF"
  | (* 70% Magenta *) (* Orchid = "#D070D0" *) Pink -> "#FFC0C0"
  | (* Hotpink = "#FF68B0" Thisle = "#D0C0D0" *) Deep_pink -> "#FF1493"
  | (* Fuchsya = FF0090 *) Gris -> "#E2E6E9"
  | (* Salmon = "#F08070" *) Beige -> "#FFCCA0"
  | Lime -> "#00FF00"
  | (* Chartreuse = "#80FF00" *) Lilac -> "#E6CCFF"
  | Violet -> "#461B7E"
  | Pale_rose -> "#FFDDDD"
  | (* Mistyrose = "#FFE4E1" *) Light_blue -> "#ADD8E6"
  | Lavender -> "#E6E6FA"
...
let roman_fonts = [ "IndUni-T"; "Arial Unicode MS" ]
and (* "Times_CSX" *) greek_fonts = [ "Arial Unicode MS"; "Symbol" ]
and (* "Latin Extended-B" Greek *) diacr_fonts =
  [ "IndUni-T"; "Arial Unicode MS" ]
and (* Sanskrit transliteration in romanised script with diacritics *)
  deva_fonts = [ "Arial Unicode MS" ]
  
(* Devanagari fonts *)
(* NB: "Devanagari MT" deprecated because wrong rendering of tacchrutvaa *)
let roman_font = Font_family roman_fonts
and greek_font = Font_family greek_fonts
and trans_font = Font_family diacr_fonts
and deva_font = Font_family deva_fonts
  
let points n = (string_of_int n) ^ "pt"
and pixels n = (string_of_int n) ^ "px"
and percent n = (string_of_int n) ^ "%"
  
let font_style =
  function | Normal -> "normal" | Italic -> "italic" | Slanted -> "oblique"
  
(* Not well-supported by browsers *)
let justify =
  function | Left -> "left" | Center -> "center" | Right -> "right"
  
(* Style sheet generator *)
let style_sheet =
  function
  | Font_family fonts ->
      let family = String.concat "," fonts in "font-family: " ^ family
...
let enpied = "position: fixed; bottom: 0pt; width: 100%"
...
let styles =
  ...
  | Pink_back -> [ Bgcolor Pale_rose; No_margin ]
  | (* Pink *) Yellow_back -> [ Bgcolor Yellow ]
  ...
  | Bandeau ->
      [ roman_font; Bgcolor Cyan; Border_sep; Border_sp 10; Full_width ]
  | Body ->
      [ roman_font; Bgcolor Pale_rose; Border_sep; Border_sp 10; Full_width ]
...
let style cla = String.concat "; " (List.map style_sheet (styles cla))
...
let span style text = (span_begin style) ^ (text ^ span_end)
...
let copyright = "© " ^ (author_name ^ (start_year ^ current_year))
  
let author = fieldn "author" author_name
...
let heritage_dictionary_title = title "Sanskrit Heritage Dictionary"
...
let target =
  match Paths.platform with
  | "Simputer" | (* Historical - small screen *) "Smartphone" | "Tablet" ->
      Simputer
  | (* TODO *) "Computer" -> Computer
  | (* Standard client installation *) "Station" -> Station
  | (* Permits external Analysis mode and User-aid *) "Server" -> Server
  | (* Http server for Internet web services *) _ ->
      failwith "Unknown target platform"
  
(* Features of target architecture *)
let (narrow_screen, screen_char_length, css) =
  match target with
  | Simputer -> (true, 40, false)
  | (* Historical for Simputer platform *)
      Station | (* Privileged client mode *) Computer | (* Shared frame *)
        Server
      -> (false, 80, true)
...
let lexicon_of =
  function | French -> "SH" | (* Sanskrit Heritage *) English -> "MW"
and (* Monier-Williams   *) language_of_string =
  function
  | "SH" -> French
  | "MW" -> English
  | _ -> failwith "Unknown lexicon"
  
let default_language = Paths.default_lexicon |> language_of_string
  
(* linked lexical resource - initialized at configuration  *)
let lexicon_toggle = ref Paths.default_lexicon
  
(* mutable for lexicon access *)
let toggle_lexicon lex = lexicon_toggle := lex
  
let page_extension lang =
  let lang_sfx = function | French -> ".fr" | English -> ".en"
  in (lang_sfx lang) ^ ".html"
  
let wrap_ext page lang = page ^ (page_extension lang)
  
let site_entry_page = wrap_ext "index"
...
let images_top_path = "IMAGES/"
  
let rel_sanskrit_page_url l = rel_dico_path ^ (site_entry_page l)
...
let rel_image name = rel_web_images_url ^ name
  
(* rel image is relative in order to pre-compile DICO in distribution site *)
let rel_ocaml_logo = rel_image "icon_ocaml.png"
...
let meta_prefix = xml_empty_with_att "meta"
  
let contents_instructions = [ [ ("charset", "utf-8") ] ]
...
(* Assuming HTML5 *)
let url dns = "http://" ^ dns
  
let ocaml_site = url "ocaml.org"
and inria_site = url "www.inria.fr/"
and tomcat = url "localhost:8080/"
  
(* Sanskrit Library runs Tomcat *)
(**********)
(* Button *)
(**********)
let js_string_arg s = let delim delim s = delim ^ (s ^ delim) in delim "'" s
  
type js_funcall = { js_funid : string; js_funargs : string list }

let string_of_js_funcall f =
  let js_funargs = List.map js_string_arg f.js_funargs
  in f.js_funid ^ ("(" ^ ((String.concat ", " js_funargs) ^ ")"))
  
let button ?id ?cl ?onclick label =
  let button = "button" in
  let attrs =
    add_opt_attrs
      [ ("onclick", (Gen.opt_app string_of_js_funcall onclick)); ("id", id);
        ("class", (Gen.opt_app class_of cl)) ]
      [] in
  let button_begin = xml_begin_with_att button attrs in
  let button_end = xml_end button in button_begin ^ (label ^ button_end)
...
let escape s =
  ...
  let subst s = (s |> Str.matched_string) |> escape
  in Str.global_substitute special_chars subst s
