...
let rec previous b left z =
  ...
  | (n, t) :: _ ->
      let w1 = extract_zip z and w2 = last_trie t in w1 @ (n :: w2)
  
(* Vicious hack to return first homonym if it exists - ugly *)
let next_trie_homo =
  let rec next_rec pref =
    function
    | Trie ((b, l)) ->
        if b
        then List.rev pref
          ...
          (try
             let _ = List.assoc 51 l
             in (* looking for homonym \#1 *) List.rev (51 :: pref)
           with
           | (* found - we know it is accepting *)
               (* no homonym - we keep looking for first accepting suffix *)
               Not_found ->
               (match l with
                | [] -> failwith "next"
                | (* should not happen if trie in normal form *) (n, u) :: _
                    -> next_rec (n :: pref) u))
  in next_rec []
  
let escape w = raise (Last (Canon.decode w))
...
let search w t =
  let rec access w t z =
    match w with
    | [] ->
        let w1 = extract_zip z and w2 = next_trie_homo t in
        let exact = w2 = []
        and homo = w2 = [ 51 ]
        in ((Canon.decode (w1 @ w2)), exact, homo)
    | n :: rest ->
        (match t with
         | Trie ((b, arcs)) ->
             (match arcs with
              | [] ->
                  if b then escape (extract_zip z) else failwith "Empty trie"
              | _ ->
                  let (left, right) = List2.zip n arcs
                  in
                    (match right with
                     | [] ->
                     ...
                     | (m, u) :: upper ->
                         if m = n
                         then access rest u (Zip ((b, left, m, upper, z)))
                         else escape (previous b left z))))
  in access w t Trie.Top
  
let read_entries () : trie = Gen.gobble Data.public_entries_file
  
let is_in_lexicon word =
  ...
  let entries_trie = read_entries () in Trie.mem word entries_trie
