...
;
(* Glitch to add certain undeclinables as ifcs by [Make_nouns] Phase 2 *)
value enter_ind_ifc entry =
  let delta = Encode.diff_str entry in fun
   [ Indecl k w -> add_morph w delta (Ind_form k)
   | _ -> failwith "Unexpected arg to enter"
   ]
;
