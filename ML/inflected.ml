...
value indifcs = ref (Deco.empty : inflected_map)
;
value add_morphindifc w d i = 
  indifcs.val := Lexmap.addl indifcs.val w (d w,i)
;
...
type flexion =
  ...
  | Indifc of ind_kind and word (* indeclinable ifc *)
...
value enter1 entry =
  ...
  and aapv = admits_aa.val (* for phantom forms generation *) in fun
   ...
   | Indecl k w -> match k with
     ...
     | Abs -> () (* only used for ifc .namul *)
     | Infl | Nota -> () (* no recording in morph tables *)
     (* Infl generated by flexions of nouns, and
        Nota are specific notations of Panini suutras *)
     ] 
   ...
   | Invar m w -> let (_,vi) = m 
     ...
     | Absoya (* abso in -ya *) -> do 
         { add_morphabsya w delta f aapv (* abs-ya: pv or gati mandatory *)
         ; if auxiliary entry then add_morphauxiinv w delta f else ()
         }
     | Absotvaa (* abso in -tvaa *) -> do 
