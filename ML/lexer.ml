...
(* word length mirror patch *)
module Lexer
  (Prel : (* takes its prelude and control arguments as module parameters *)
  ...
  struct
    ...
    (* [morpho tag_sort tags_of] *)
    open Load_transducers
      
    (* [transducer_vect Trans] *)
    ...
    let print_tags pvs seg_num phase form tags =
      let ptag =
        print_morph pvs (is_cache phase) seg_num (generative phase) form in
    ...
    let rec scl_phase =
      ...
      | Iic | A | An | Iicv | Iicc | Iik | Iikv | Iikc | Iiif | Auxiick | Ai
          | Ani | Cachei -> "iic"
      | Iiv | Iivv | Iivc -> "iiv"
      | Iiy -> "iiy"
      | Peri -> "peri"
    ...
    let print_scl_morph pvs gen form tag =
      ((xml_begin "tag") |> ps;
       Morpho_scl.print_scl_inflected pvs form gen tag;
       (xml_end "tag") |> ps)
      
    let print_scl_tags pvs phase form tags =
      let table phase =
        xml_begin_with_att "tags" [ ("phase", (scl_phase phase)) ]
      in
        ((table phase) |> ps;
   ...
   Lexer, but not [Load_morphs] or Dispatcher. Modules ought to be revised. *)
    ...
    let process_transition =
      ...
      | Euphony ((w, u, v)) ->
          let off =
            if w = [] then 1 else (* amui/lopa from Lopa/Lopak *) length w
          in off - ((length u) + (length v))
      | Id -> 0
    ...
    let process_kridanta pvs seg_num phase form tags =
      (th_begin |> ps;
       (table_morph_of phase) |> pl;
       (* table begin *)
       let ok_tags =
         if pvs = []
         then tags
    ...
    (* value used by [Parser.print_segment_roles] *)
    ...
    let print_morpho phase word =
      ((table_morph_of phase) |> pl;
       ...
       (let _ =
          match tags_of phase word with
          | Atomic tags -> process_kridanta [] 0 phase word tags
          | Preverbed ((_, phase), pvs, form, tags) ->
    ...
    let print_segment offset (phase, rword, transition) =
      ("[ " |> ps;
       ...
       "&lang;" |> ps;
       (* < *)
       let correction = process_transition transition
       in
    ...
    let print_scl_segment counter (phase, rword) =
      let word = Morpho_html.visargify rword
      in
        ((let solid = background (Phases.color_of_phase phase)
          in (td_begin_class solid) |> pl);
         (let ic = string_of_int counter
          in
         ...
         (match tags_of phase (mirror rword) with
          | Atomic tags -> print_scl_tags [] phase word tags
          | Preverbed ((_, phase), pvs, form, tags) ->
              let ok_tags =
                if pvs = []
                then tags
                else
                  trim_tags (generative phase) form (Canon.decode pvs) tags
              in print_scl_tags pvs phase form ok_tags);
         "'>" |> ps;
         (* closes <input *)
