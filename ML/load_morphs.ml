...
module Morphs
  (Prel : sig val prelude : unit -> unit
                 end)
  (Phases :
    sig
      type phase =
        ...
        | Nouc
        | Krid
        | (* K.ridaantaas - used to be called Parts *)
        Vok
        | (* K.ridaanta vocatives *)
        Iik
        | (* K.ridaantaas as left component - used to be called Piic *)
        Iikv
        | Iikc
        | Kriv
        | Kric
        ...
        | Kama
        | Cache
        | (* Cached lexicon acquisitions *)
        Cachei
        | (* Cached iic lexicon acquisitions *)
        Unknown
        | (* Unrecognized chunk *)
        Comp of (phase * phase) * (* pv *) Word.word * (* root form *) Word.
                word
...
by Dispatcher. Preverbed segments may be finite verb forms or kridantas. *)
    type tag_sort =
      | Atomic of lemmas
      | Preverbed of (phase * phase) * (* pv *) Word.word * Word.word
                     * lemmas
    
    (* Fake tags of nan prefixes *)
    let nan_prefix = Bare_stem
      
    let a_tag = [ ((0, []), [ nan_prefix ]) ]
    and an_tag = [ ((0, [ 51 ]), [ nan_prefix ]) ]
      
    (* since lexicalized as an\#1 *)
    (* [an_tag] has delta = (0,[51]) since an\#1 is the relevant entry. Such values
ought to be parameters of the specific lexicon used. *)
    (*i TODO i*)
    let ai_tag = a_tag
    and (* special for privative abs-tvaa eg akritvaa *) ani_tag = an_tag
      
    let unknown_tag = [ ((0, []), [ Unanalysed ]) ]
      
    let give_up cat =
      let mess = "Missing " ^ (cat ^ " morphology bank")
      in
        (Web.abort Html.default_language "System error - please report - "
           mess;
         Deco.
         empty)
    ...
    let load_morpho file =
      ...
      with | _ -> (Prel.prelude (); give_up file)
    and load_morpho_cache file =
      try (Gen.gobble file : inflected_map) with | _ -> Deco.empty
      
    (* dummy empty morpho lexmap *)
    ...
    let tags_of phase word =
      ...
      | Comp ((((_, ph) as sort)), pv, form) ->
          let tag = Deco.assoc form (morpho_tags ph)
          in Preverbed (sort, pv, form, tag)
