...
let make_inflected inflected_file =
  try
    let inflected : Morphology.inflected_map = Gen.gobble inflected_file
    in Mini.minimize (Deco.forget_deco inflected)
  with
  | Sys_error s ->
      ((let mess =
          s ^ ("\n\n *** Inflected file missing ***\n" ^ inflected_file)
        in output_string stderr mess);
       flush stderr;
       failwith "Make_inflected")
  
let make_preverbs preverbs_file =
  try
    let preverbs : Word.word Deco.deco = Gen.gobble preverbs_file
    in (* minimize as dag *) Mini.minimize (Deco.forget_deco preverbs)
  with
  | Sys_error s ->
      ((let mess =
...
let transducer_of_lemmas deco =
  (make_inflected deco) |> Make_automaton.make_transducer
and transducer_of_preverbs = (* special sandhi rules *)
  (make_preverbs Data.preverbs_file) |> Make_preverb_automaton.
    make_transducer
and empty_trans = State ((false, [], []))
  
(* dummy empty transducer *)
let make_transducers =
  ...
  and vocaf = transducer_of_lemmas Data.vocaf_file
