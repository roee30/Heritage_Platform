...
let main =
  ...
  let permission =
    (Cgi.decoded_get Mkdir_corpus_params.permission "" env) |> Web_corpus.
    ...
    | Web_corpus.Manager ->
        (try
         ...
         | Unix.Unix_error ((err, func, arg)) ->
             let submsg =
               Printf.sprintf "'%s' failed on '%s': %s" func arg
                 (Unix.error_message err)
             in error_page Control.sys_err_mess submsg
         | _ ->
             abort Html.default_language Control.fatal_err_mess
               "Unexpected anomaly")
    | Web_corpus.Reader | Web_corpus.Annotator ->
        let expected_permission = let open Web_corpus
          in string_of_permission Manager in
        let current_permission = Web_corpus.string_of_permission permission
        in
