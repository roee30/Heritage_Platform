(**************************************************************************)
(*                                                                        *)
(*                     The Sanskrit Heritage Platform                     *)
(*                                                                        *)
(*                              Gérard Huet                               *)
(*                                                                        *)
(* ©2019 Institut National de Recherche en Informatique et en Automatique *)
(**************************************************************************)
(* Linearizes morphological information as a string.
   Used in [Morpho, Morpho_tex, Lexer]. *)
open Skt_morph
  
open Morphology
  
(* [inflected], [Noun_form], ... *)
let gana_str k =
  if k = 11
  then " [vn.]"
  else
    if k > 10
    then (* redundant with conjugation *) ""
    else
      if k = 0 then failwith "gana_str" else " [" ^ ((string_of_int k) ^ "]")
  
let string_voice =
  function | Active -> " ac." | Middle -> " md." | Passive -> " ps."
and string_conjugation =
  function
  | Primary -> ""
  | Causative -> "ca. "
  | Intensive -> "int. "
  | Desiderative -> "des. "
and string_nominal =
  function
  | Ppp -> "pp."
  | Pppa -> "ppa."
  | Ppra k -> "ppr." ^ ((gana_str k) ^ " ac.")
  | Pprm k -> "ppr." ^ ((gana_str k) ^ " md.")
  | Pprp -> "ppr." ^ " ps."
  | Ppfta -> "ppf." ^ " ac."
  | Ppftm -> "ppf." ^ " md."
  | Pfuta -> "pfu." ^ " ac."
  | Pfutm -> "pfu." ^ " md."
  | Pfutp k -> "pfp." ^ (gana_str k)
  | Action_noun -> "act."
and string_tense =
  function
  | Future -> "fut."
  | Perfect -> "pft."
  | Aorist k -> "aor." ^ (gana_str k)
  | Injunctive k -> "inj." ^ (gana_str k)
  | Conditional -> "cond."
  | Benedictive -> "ben."
  | Subjunctive -> "subj."
and string_case =
  function
  | Nom -> "nom."
  | Acc -> "acc."
  | Ins -> "i."
  | Dat -> "dat."
  | Abl -> "abl."
  | Gen -> "g."
  | Loc -> "loc."
  | Voc -> "voc."
and string_number =
  function | Singular -> " sg. " | Dual -> " du. " | Plural -> " pl. "
and string_gender =
  function | Mas -> "m." | Neu -> "n." | Fem -> "f." | Deictic _ -> "*"
and string_pr_mode =
  function
  | Present -> "pr."
  | Imperative -> "imp."
  | Optative -> "opt."
  | Imperfect -> "impft."
and string_person = function | First -> "1" | Second -> "2" | Third -> "3"
and string_ind_kind =
  function
  | Part -> "part."
  | Prep -> "prep."
  | Conj -> "conj."
  | Abs -> "abs."
  | Adv -> "adv."
  | Tas -> "tasil"
  | _ -> "ind."
and string_invar =
  function | Infi -> "inf." | Absoya -> "abs." | Perpft -> "per. pft."
  
let string_paradigm =
  function
  | Conjug (t, v) -> (string_tense t) ^ (string_voice v)
  | Presenta (k, pr) -> (string_pr_mode pr) ^ ((gana_str k) ^ " ac.")
  | Presentm (k, pr) -> (string_pr_mode pr) ^ ((gana_str k) ^ " md.")
  | Presentp pr -> (string_pr_mode pr) ^ " ps."
  | Perfut v -> "per. fut." ^ (string_voice v)
  
let string_finite (c, p) = (string_conjugation c) ^ (string_paradigm p)
and string_verbal (c, n) = (string_conjugation c) ^ (string_nominal n)
and string_modal (c, i) = (string_conjugation c) ^ (string_invar i)
  
let string_morph =
  function
  | Noun_form (g, n, c) | Part_form (_, g, n, c) ->
      (string_gender g) ^ ((string_number n) ^ (string_case c))
  | Bare_stem | Avyayai_form -> "iic."
  | Avyayaf_form -> "ind."
  | Verb_form (f, n, p) ->
      (string_finite f) ^ ((string_number n) ^ (string_person p))
  | Ind_form k -> string_ind_kind k
  | Abs_root c -> (string_conjugation c) ^ "abs."
  | Gati -> "iiv."
  | Ind_verb m -> string_modal m
  | Unanalysed -> "?"
  | PV pvs -> "pv."
  

