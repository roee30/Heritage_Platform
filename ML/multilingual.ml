...
let gender_caption gender =
  ...
  | Deva ->
      deva12_blue_center
        (Encode.skt_raw_to_deva
           (match gender with
            | Mas -> "pumaan"
            | Fem -> "strii"
            | Neu -> "napu.msakam"
            | Deictic _ -> "sarvam"))
and number_caption number =
  function
  | Roma ->
      span3_center
        (match number with
...
type gentense = | Present_tense of pr_mode | Other_tense of tense

let tense_name gentense =
  function
  | Deva ->
      let s =
        (match gentense with
      ...
      in deva16_blue_center (Encode.skt_raw_to_deva s)
  | Roma ->
      let s =
        (match gentense with
  ...
  | Deva ->
      let deva_person =
        (match person with
         ...
         | Second -> "madhyama"
         | Third -> "prathama")
      in deva12_blue_center (Encode.skt_raw_to_deva deva_person)
  | Roma ->
      let roma_person =
        (match person with
         | First -> "First"
         | Second -> "Second"
         | Third -> "Third")
      in span3_center roma_person
  
let conjugation_name conj =
  function
  | Deva ->
      let indian_conj =
        (match conj with
         ...
         | Intensive -> "yaf"
         | Desiderative -> "san")
      in deva16_blue_center (Encode.skt_raw_to_deva indian_conj)
  | Roma ->
      let western_conj =
        (match conj with
         | Primary -> "Primary"
      ...
      in span2_center (western_conj ^ " Conjugation")
  
let conjugation_title narrow =
  function
...
let voice_mark =
  function | Active -> "para" | Middle -> "aatma" | Passive -> "karma.ni"
  
let participle_name part =
  function
  | Deva ->
      let indian_part =
        (match part with
         ...
         | Pfutp k ->
             (match k with
              ...
              | _ -> [])
         | Action_noun -> [ "krit" ]) in
      (* "gha~n" for -a "lyu.t" for -ana *)
      let cat s x = s ^ (" " ^ (Encode.skt_raw_to_deva x))
      in List.fold_left cat "" indian_part
  | (* no skt punctuation so far *) Roma ->
      let western_part =
        (match part with
  ...
  | Deva ->
      let ivoice =
        (match voice with
         ...
         | Middle -> "aatmanepade"
         | Passive -> "karma.ni")
      in deva12_blue_center (Encode.skt_raw_to_deva ivoice)
  | Roma ->
      let wvoice =
        (match voice with
         | Active -> "Active"
