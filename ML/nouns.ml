...
(* ; Bare Noun (wrap stem 3) eg kumaarimataa Pan{6,3,42} *)
   ; Avyayaf (wrap stem 3) 
   ]
;
...
value build_tvad () = 
  ...
  enter "yu.smad" (* Paninian entry *)
   ...
   ; Bare Pron (code "tvad") (* \Pan{7,2,98} when meaning is singular *)
...
(* privative "a/an1" not included because generated in Complete mode,
   ...
   but aa- prefixes not generative - cpd must be lexicalized (aaniila). *)
  ]
