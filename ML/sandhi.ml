...
let visargcomp1 first =
  function
  | [] -> failwith "left arg of sandhi too short (1)"
  | penu :: _ ->
      (match penu with
       | 1 -> [ (-1); 12; first ]
       | (* o  -1 means erase a *) 2 -> [ first ]
       | (* visarga dropped after aa *) _ -> [ 43; first ])
  
(* visarga goes to r *)
let visargcomp2 =
  function
  | (* first = `r`, visarga goes to r *) [] ->
      raise (Failure "left arg of sandhi too short (2)")
  | penu :: _ ->
      (match penu with
       | 1 -> [ (-1); 12; 43 ]
       | (* "a.h+r {\R} or"  -1 means erase a *) 2 -> [ 43 ]
       | (* "aa.h+r {\R} aar" *) c -> [ (-1); long c; 43 ])
  ...
  | penu :: _ ->
      (match penu with
       | 1 ->
           if first = 1
           then [ (-1); 12; (-1) ]
           else (* erase a, o, then avagraha *) [ 50; first ]
       | (* hiatus *) 2 -> [ 50; first ]
       | (* hiatus *) c -> [ 43; first ])
  ...
  | last :: before ->
      (match wr with
       | (* Nota Bene : we assume [wl] to be in final sandhi form except r or s.    *)
           ...
           [] -> (wl, [])
       | (* no visarga for final s or r *) first :: after ->
           if vowel last
           then
             if vowel_or_phantom first
             then (* first may be *e or *o, thus uph below *)
               (let glue =
                          ...
                          (match last with
                           ...
                           | (* .r {\R} r *) 10 | 12 -> (* e o *)
                               ...
                               then [ last; (-1) ]
                                 ...
                                 if first = (-11)
                                 then [ 1; if last = 10 then 42 else 44; 2 ]
                                 else
                           ...
                           | (* aav *) _ ->
                               let message =
                                 "left arg of sandhi end illegal in "
                               in failwith (message ^ (decode wl)))
                in (before, (glue @ after)))
             else
               (wl, (if first = 23 then (* ch *) 22 :: wr else (* cch *) wr))
           else
             (* c optional except when [short_vowel last] or wl=\=a or m\=a *)
             (* we assume that last cannot be a phantom and thus is a consonant *)
             (let glue =
                if vowel first
                then
                  if visarg last
                  then visargcompv first before
                  else (* may start with -1 *)
                    (match last with
                     | 21 ->
                         (match before with
                          ...
                          | v :: rest ->
                              if short_vowel v
                              then 21 :: 21 :: (uph first)
                              else (* ff *) 21 :: (uph first))
                     | 36 ->
                         (match before with
                          | [] -> failwith "left arg too short"
                          | v :: rest ->
                              if short_vowel v
                              then 36 :: 36 :: (uph first)
                              else (* nn *) 36 :: (uph first))
                     | c -> (voiced c) :: (uph first))
                else (* t {\R} d, p {\R} b  *) (* both consonant *)
                  (match first with
                   | 49 -> (* h *)
                       if visarg last
                       then visargcomp1 first before
                       else
                         (match last with
                          | 17 | 19 -> [ 19; 20 ]
                          | (* k+h {\R} ggh,  g+h {\R} ggh *) 27 ->
                              [ 29; 30 ]
                          | (* {\d t}+h {\R} {\d d}{\d d}h *) 32 | 34 ->
                              [ 34; 35 ]
                          | (* t+h {\R} ddh,  d+h {\R} ddh *) 37 | 39 ->
                              [ 39; 40 ]
                          | (* p+h {\R} bbh,  b+h {\R} bbh *) 41 ->
                              [ 14; first ]
                          | (* m+h {\R} {\d m}h *)
                   ...
                   | 46 -> (* \'s *)
                       (match last with
                        ...
                        | (* t+\'s {\R} cch  idem d c *)
                            ...
                            36 -> [ 26; 23 ]
                        | (* n+\'s {\R} \~nch (or [ 26; 46 ] \~n\'s) *) 41 ->
                            [ 14; first ]
                        | (* m+\'s {\R} {\d m}\'s (or \~nch optional) *) c ->
                            [ if visargor c then 16 else c; first ])
                   | 36 | 41 -> (* n m *)
                       if visarg last
                       then visargcomp1 first before
                       else
                         (match last with
                          | 17 | 21 -> [ 21; first ]
                          | (* k+n {\R} \.nn 'n+n -> 'nn *) 27 | 29 ->
                              [ 31; first ]
                          | (* {\d t}+n {\R} {\d n}n  {\d d}+n {\R} {\d n}n *)
                              32 | 34 -> [ 36; first ]
                          | (* t+n {\R} nn  d+n {\R} nn *) 37 ->
                              [ 41; first ]
                          | (* p+n {\R} mn *) 41 -> [ 14; first ]
                          | (* m+n {\R} {\d m}n *) c -> [ c; first ])
                   | (* \.n+n {\R} \.nn etc. *) 47 | 48 -> (* {\d s} s *)
                       (match last with
                        | 41 -> [ 14; first ]
                        | (* m+s {\R} {\d m}s *) 34 -> [ 32; first ]
                        | (* d+s {\R} ts *) c ->
                            [ if visargor c then 16 else c; first ])
                   | 37 | 38 | 17 | 18 -> (* p ph k kh *)
                       (match last with
                        | 41 -> [ 14; first ]
                        | (* m+p {\R} {\d m}p *) 34 -> [ 32; first ]
                        | (* d+p {\R} tp *) c ->
                            [ if visargor c then 16 else c; first ])
                   | (* s+k {\R} {\d h}k but optional {\d s}k *) 44 ->
                       (* l *)
                       if visarg last
                       then visargcomp1 first before
                       else
                         (match last with
                          | 32 | 34 -> [ 44; 44 ]
                          | (* t+l {\R} ll  d+l {\R} ll *) 36 | 41 ->
                              [ 44; 15; 44 ]
                          | (* n+l {\R} l\~ l (candrabindu) *) c ->
                              [ voiced c; 44 ])
                   | 42 | 45 -> (* y v *)
                       if visarg last
                       then visargcomp1 first before
                         ...
                         (match last with
                          | 41 -> [ 14; first ]
                          | (* m+y {\R} {\d m}y *) c -> [ voiced c; first ])
                   | 43 -> (* r *)
                       if visarg last
                       then visargcomp2 before
                       else
                         (match last with
                          | 41 -> [ 14; 43 ]
                          | (* m+r {\R} {\d m}r *) 43 -> visargcompr before
                          | (* Gonda §16 *) c -> [ voiced c; first ])
                   ...
                   | 29 | 30 -> (* {\d d} {\d d}h *)
                       ...
                       then visargcomp1 first before
                         ...
                         (match last with
                          | 41 -> [ 14; first ]
                          | (* m+{\d d} {\R} {\d m}{\d d} == {\d n}{\d d} *)
                              32 | 34 -> [ 29; first ]
                          | (* t+{\d d} {\R} {\d d}{\d d}  d+{\d d} {\R} {\d d}{\d d} *)
                              36 -> [ 31; first ]
                          | (* n+{\d d} {\R} {\d n}{\d d} *) c ->
                              [ voiced c; first ])
                   | 24 | 25 -> (* j jh *)
                       ...
                       then visargcomp1 first before
                         ...
                         (match last with
                          | 41 -> [ 14; first ]
                          | (* m+j {\R} {\d m}j == \~nj *) 32 | 34 ->
                              [ 24; first ]
                          | (* t+j {\R} jj  d+j {\R} jj *) 36 ->
                              [ 26; first ]
                   ...
                   | 32 | 33 -> (* t th *)
                       (match last with
                        ...
                        | (* m+t {\R} {\d m}t == nt *) 36 ->
                            [ 14; 48; first ]
                        | (* n+t {\R} {\d m}st *) 34 -> [ 32; first ]
                        | (* d+t {\R} tt *) c ->
                            [ if visargor c then 48 else c; first ])
                   | (* s+t {\R} st *) 27 | 28 -> (* {\d t} {\d t}h *)
                       (match last with
                        | 41 -> [ 14; first ]
                        | (* m+{\d t} {\R} {\d m}{\d t} == {\d n}{\d t} *)
                            32 | 34 -> [ 27; first ]
                        | (* t+{\d t} {\R} {\d t}{\d t}  d+{\d t} {\R} {\d t}{\d t} *)
                            36 -> [ 14; 47; first ]
                        | (* n+{\d t} {\R} {\d m}{\d s}{\d t} *) c ->
                            [ if visargor c then 47 else c; first ])
                   | 22 | 23 -> (* c ch *)
                       (match last with
                        | 41 -> [ 14; first ]
                        | (* m+c {\R} {\d m}c == \~nc *) 32 | 34 ->
                            [ 22; first ]
                        | (* t+c {\R} cc  d+c {\R} cc *) 36 ->
                            [ 14; 46; first ]
                        | (* n+c {\R} {\d m}\'sc *) c ->
                            [ if visargor c then 46 else c; first ])
                   | (* | 31 (* {\d n} *) missing c+.n = f.n TODO *) c ->
                       failwith
                         ("illegal start of right arg of sandhi in " ^
                            (decode wr))) in
              (* match first *) (* let glue *)
              let (w1, w2) =
                match glue with
                | [] -> failwith "empty glue"
                | (-1) :: rest ->
...
(* match [wr] *)
(* match [wl] *)
let ext_sandhi0 wl wr = (* No normalization *)
  let (w1, w2) = ext_sandhi_pair wl wr in List2.unstack w1 w2
  
(* w1 is pasted as left context of w2 *)
(* Only used in stand-alone module Sandhier; argument is rev of word *)
let final_sandhi =
  function
  | [] -> failwith "Empty input Sandhi"
  | last :: rest when visargor last -> List.rev (16 :: rest)
  | (* final visarga *) rw -> List.rev (finalize rw)
  
(* External sandhi - Reference version - used in [Roots.follow] *)
(* [esandhi : string -> string -> word] *)
let esandhi left right =
  let wl = List.rev (code left)
  and wr = code right
  in Encode.normalize (ext_sandhi0 wl wr)
  
(* normalization *)
(* Unused directly; copied in [Compile_sandhi.match_sandhi] *)
(* [e_sandhi : string -> string -> string] *)
let e_sandhi left right = decode (esandhi left right)
  
(* Used in [Roots.follow] and [Make_preverbs.preverbs_etym] *)
let pv_sandhi left right =
  if (left = "pra") && (right = "ni")
  then "pra.ni"
  else (* retroflexion *) e_sandhi left right
and pv_sandhi0 wl wr =
  let rwl = Word.mirror wl
  in
    if (rwl = (code "pra")) && (wr = (code "ni"))
    then code "pra.ni"
    else (* retroflexion *) Encode.normalize (ext_sandhi0 wl wr)
...
let _ = assert ((e_sandhi "tat" "lebhe") = "tallebhe")
  
let _ = assert ((e_sandhi "tat" "zrutvaa") = "tacchrutvaa")
...
let _ = assert ((e_sandhi "punar" "suuti") = "puna.hsuuti")
...
let _ = assert ((e_sandhi "sam" "hraad") = "sa.mhraad")
...
let _ = assert ((e_sandhi "bahis" "k.r") = "bahi.hk.r")
...
let _ = assert ((e_sandhi "kim" "hmalayati") = "ki.mhmalayati")
  
(* but "kimhmalayati" also correct *)
let _ = assert ((e_sandhi "kim" "hnute") = "ki.mhnute")
  
(* but "kinhnute" also correct (metathesis) *)
let _ = assert ((e_sandhi "tat" "mitram") = "tanmitram")
...
let _ = assert ((e_sandhi "iha" "aa|ihi") = "ihehi")
  
(* e-phantom elim *)
let _ = assert ((e_sandhi "iha" "aa") = "ihaa")
  
let _ = assert ((e_sandhi "ihaa" "ihi") = "ihehi")
  
(* Idem for *o : fake sandhi "aa" "upa" = "aa|upa") generated by Inflected. *)
let _ = assert ((e_sandhi "zoka" "aa|rta") = "zokaarta")
  
(* Context-sensitive irregularities *)
let external_sandhi left right =
  if (left = "sas") || (left = "sa.h")
  then
...
let ext_sandhi rvlword rword =
  let left =
    match rvlword with
    | [ 48; 1; 48 ] | [ 16; 1; 48 ] ->
        (match rword with
         | [] -> [ 16; 1; 48 ]
         | first :: after -> if vowel first then [ 16; 1; 48 ] else [ 1; 48 ])
    | l -> l
  in ext_sandhi0 left rword
  
(* does not finalize r or s into .h *)
let after_dual_sandhi left right =
  match List.rev (code left) with
  | [] -> failwith "left arg of sandhi empty"
  | last :: _ ->
      if (last = 4) || (* ii *) ((last = 6) || (* uu *) (last = 10))
      then (* e *) left ^ ("_" ^ right)
      else (* hiatus *) e_sandhi left right
