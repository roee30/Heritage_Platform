...
let title =
  h1_title (if narrow_screen then "Sandhi" else "The Sandhi Engine")
  ...
  (pl http_header;
   ...
   in
     try
       ...
       let left_word = encode left_str and right_word = encode right_str in
       let rleft_word = Word.mirror left_word and final = right_word = [] in
       let result_word =
         match url_encoded_kind with
         | "external" ->
             if final
             then final_sandhi rleft_word
             else ext_sandhi rleft_word right_word
         | "internal" ->
             if final
             then raise (Control.Fatal "Empty right component")
             else int_sandhi rleft_word right_word
         | _ -> raise (Control.Fatal "Unexpected kind") in
       let kind = if final then "final" else url_encoded_kind in
       let left = Canon.decode left_word
       and (* = [left_str] *) right = Canon.decode right_word
       and (* = [right_str] *) result = Canon.decode result_word
       in
         (ps (span_begin C1);
          ps ("The " ^ (kind ^ " sandhi of "));
          ps (display_rom_red left);
          if final then () else (ps " and "; ps (display_rom_red right));
          ps " is ";
...
let safe_engine () =
  ...
  in
    ...
    | (* file pb *) Encode.In_error s -> abor "Wrong_input " s
    ...
    | (* EOF *) Control.Fatal s -> abor "Wrong parameters " s
    | Exit -> abor "Wrong character in input - " "check input convention"
