...
let confirmation_page query =
  ...
  let confirmation_msg =
    Printf.sprintf "Confirm changes for sentence no. %s of %s ?" sentno
      corpdir in
  let specific_url path = Cgi.url path ~fragment: sentno
  in
    (maybe_http_header ();
...
let analysis_of_env env =
  let lang =
    (env |> (Cgi.decoded_get "lex" Paths.default_lexicon)) |> Html.
      language_of_string in
  let cpts = env |> (Cgi.decoded_get "cpts" "") in
  (* [|> Checkpoints.parse_cpts] *)
  let nb_sols =
    (env |> (Cgi.decoded_get Save_corpus_params.nb_sols "0")) |>
      int_of_string
  in Corpus.Analysis.make Corpus.Analyzer.Graph lang cpts nb_sols
  
let error_page = error_page "Corpus Manager"
...
let main =
  ...
  let env = Cgi.create_env query in
  let query = Cgi.decoded_get Save_corpus_params.state "" env
  in
    try
      let force =
        (env |>
      ...
      let corpdir = Cgi.decoded_get Params.corpus_dir "" env in
      let sentno =
        ((env |> (Cgi.decoded_get Params.sentence_no "")) |> float_of_string)
          |> int_of_float in
      let text = Cgi.decoded_get "text" "" env in
      let unsandhied = (Cgi.decoded_get "us" "f" env) = "t" in
      let permission =
        Web_corpus.permission_of_string
          (Cgi.decoded_get Params.corpus_permission "" env)
      in
        match permission with
        | Web_corpus.Annotator ->
            let read_skt = Sanskrit.read_raw_sanskrit_corpus in
            (* NEW 2020 *)
        ...
        | Web_corpus.Reader | Web_corpus.Manager ->
            let expected_permission =
              Web_corpus.string_of_permission Annotator in
            let current_permission =
              Web_corpus.string_of_permission permission
            in
              invalid_corpus_permission_page expected_permission
                current_permission
