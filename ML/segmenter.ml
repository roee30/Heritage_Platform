...
module Segment
  ...
  struct
    ...
    let max_input_length = 600
      
    type phased_padas =
      (phase * (Word.word list))
      and (* padas of given phase *)
      segments =
      phased_padas list
    ...
    let all_checks = ref ([] : check list)
    and (* checkpoints in rest of input *) offset_chunk = ref 0
    ...
    let set_offset (offset, checkpoints) =
      (offset_chunk := offset; all_checks := checkpoints)
      
    let set_sa_control b = sa_control := b
   ...
   by [react], with proper pruning of backtracking. *)
    let check_chunk solution =
      let position = !offset_chunk and checkpoints = !all_checks in
      ...
      in check_rec position (List.rev solution) checkpoints
...
*)
      ...
      | 123 :: (* *C *) r ->
          (match previous_segments with
           | (phase, rword, Euphony ((_, u, [ 123 ]))) :: rest ->
               let w = sandhi_aa u in
               let new_segment = (ph, (Word.mirror (23 :: r)), rule)
               in
                 new_segment ::
                   ((aa_phase ph), [ 2 ],
    ...
    let schedule phase input output w cont =
      ...
      let transitions =
        ...
        then []
        else (* Word = Sanskrit pada *) dispatch w phase
      in
        (* iterate Word+ = Sanskrit vaakya *)
          ...
          (match resume with
           ...
           | Advance (phase, input, output, occ) ->
               (match access phase occ with
                | None -> continue back
                | Some ((next_state, v)) ->
                    react phase input output back v next_state))
    and emit solution cont =
      if check_chunk solution
      then
        (match sanitize_sa !sa_control solution with
         | [] -> continue cont
         | ok -> Some ((ok, cont)))
      else (* solution found *) continue cont
      
    let init_segment_initial initial_phases sentence =
      List.map (fun phase -> Advance (phase, sentence, [], []))
        initial_phases
      
    let segment1_initial initial_phases sentence =
      continue (init_segment_initial initial_phases sentence)
      
    let init_segment seg = (* do not eta reduce! *)
      init_segment_initial initial seg
    and segment1 seg = segment1_initial initial seg
