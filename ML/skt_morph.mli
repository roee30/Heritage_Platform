...
type gender =
  | Mas | Neu | Fem | Deictic of deictic
  and genders =
  gender list
...
type gana =
  int
  and (* present class: 1 to 10, plus 11 for denominatives *)
  aor_class =
  int
...
type finite =
  ...
  and (* finite forms of verbs *)
  paradigm =
  | Presenta of gana * pr_mode
  | (* parasmaipade *)
  Presentm of gana * pr_mode
  | (* aatmanepade *)
  Presentp of pr_mode
  | (* passive of present system *)
  Conjug of tense * voice
  | (* other tenses/modes/aspects *)
  Perfut of voice
  and (* periphrastic futur (lu.t) - always active *)
  voice =
  | Active | Middle | Passive
  and (* diathesis (pada: Para Atma Ubha) *)
  pr_mode =
  | Present
  | (* Indicative (la.t) *)
  Imperfect
  ...
  | (* (l.r.t) *)
  Perfect
  | (* Remote past - resultative aspect (li.t) *)
  Aorist of aor_class
  | (* Immediate past or future with perfective aspect (luf) *)
  Injunctive of aor_class
  | (* (le.t) - injunctions also Prohibitive with maa *)
  Benedictive
  | (* Precative: optative aorist (aazirlif) *)
  Conditional
  | (* Preterit of future (l.rf) *)
  Subjunctive

(* le.t *)
(* Rare subjunctive, intermediate between Optative and Imperative *)
...
(* shades of intention of passive future/potential participle: 
...
type verbal =
  ...
  Ppftm
  | (* middle perfect participle *)
  (* no passive *)
  Pfuta
  | (* active future participle *)
  Pfutm
  | (* middle future participle *)
  Pfutp of kritya
  | (* passive future/potential participle/gerundive 3 forms *)
  Action_noun

(* generative only for auxiliaries, for cvi compounds *)
...
type modal =
  ...
  | (* infinitive (tumun) *)
  Absoya
  | (* absolutive (gerund, invariable participle) (lyap) *)
  Perpft
...
type nan_kind =
  | Neg
  | (* logical negation: adj -> adj *)
  Not
  | (* sentential negation: adv -> adv *)
  Opp
  | (* opposite notion: subst -> subst preserving gender *)
  Priv
  | (* bahuvrihi: noun -> adj with gender-raising *)
  Abse

(* noun -> noun in n. *)
type sadhana =
  (* karaka, action or absolutive - coarser than krit *)
  | Agent
  | Action
  | Object
  | Instr
...
type nominal =
  (conjugation * krit)
  and krit =
  (* coarser than Paninian krit suffixes *)
  | Agent_aka
  | (* .nvul \Pan{3,1,133} \Pan{3,3,108-109} -aka -ikaa v.rddhi 
                 .svun \Pan{3,1,145} trade gu.na f. -akii  
                 vu~n \Pan{3,1,146-147} 
                 vun \Pan{3,1,149-150} repeated action, benediction *)
  Agent_in
  | (* .nini \Pan{3,1,134} \Pan{3,2,78-86}   -in  -inii v.rddhi 
                 ghinu.n \Pan{3,2,141-145} 
                 ini \Pan{3,2,93} ifc. -vikrayin past *)
  Agent_tri
  | (* t.rc \Pan{3,1,133} t.rn \Pan{3,2,135} habit -t.r gu.na  *)
  Agent_ana
  | (* lyu \Pan{3,1,134} yuc \Pan{3,2,148}     -ana a.   
                 .nyu.t \Pan{3,1,147-148} profession f. -anii *)
  Agent_root
  | (* kvip \Pan{3,2,61} ifc + \Pan{3,2,76} adja  
  ...
  Action_ana
  | (* lyu.t \Pan{3,3,115-117}            -ana n.    *)
  Action_na
  | (* naf \Pan{3,3,90} nan \Pan{3,3,91} -na m. -naa f. *)
  Action_a
  | (* gha~n \Pan{3,3,18-}                -a m. v.rddhi *)
  Action_ya
  | (* kyap \Pan{3,1,107} -ya n. -yaa f.              *)
  Action_ti
  | (* ktin \Pan{3,3,94-96}                      -ti f.  *)
  Action_i
  | (* ki \Pan{3,3,92-93}                     -i f.    *)
  Action_root
  | (* unknown krit of non-agent noun  *)
  Object_root
  | (* we should probably lump action and object in [Non_agent] *)
  Object_a
  | (* ka                                     -a n.    *)
  Instrument
  | (* ka \Pan{3,1,136}                  0/amui n.   *)
  Instra
  | (* .s.tran -tra n.                -trii f. traa f.   *)
  Orig_root
  | (* sruc srut sruva *)
  Agent_u
  | (* san+u                             -u on des stem  *)
  Action_aa
  | (* san+a+.taap  \Pan{3,3,102}        -aa on des stem *)
  Abstract
...
type ind_kind =
  ...
  | (* adverb *)
  Avya
  | (* turned into an adverb by avyayiibhaava compounding *)
  Abs
  | (* absolutive *)
  Tas
  | (* tasil taddhita *)
