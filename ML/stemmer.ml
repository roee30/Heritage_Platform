...
(* [inflected inflected_map] *)
open Cgi
  
open Segmenter
  
(* [segment1_initial continue] *)
open Dispatch
  
(* Phases nominal verbal etc *)
open Auto.Auto
  
let give_up cat =
  let mess = "Missing " ^ (cat ^ " automaton")
  in failwith ("System error - please report - " ^ mess)
  
(* installation problem -- executing process fails *)
let full = false
  
(* how to call Dispatch with parameters Full=False/True? *)
...
module Control = struct let star = ref false
                           end
...
let load_morpho file =
  try (Gen.gobble file : inflected_map) with | _ -> give_up "morphology"
and load_preverbs file =
  try (Gen.gobble file : Word.word Deco.deco) with | _ -> give_up "preverbs"
...
let morph = load_morphs ()
  
(* loading morphology databases *)
exception Solution of Word.word
  
exception Solution2 of Word.word * Word.word
  
exception Finished
  
(* looks for first segmentation in one segment *)
let rec search_unit_solution (output, cont) =
  match List.rev output with
  | [ (_, word, _) ] -> raise (Solution word)
  | (* one segment *) [ (_, word1, _); (_, word2, _) ] ->
      raise (Solution2 (word1, word2))
  | (* 2 segments *) _ ->
      (match continue cont with
       | (* Note : continue is generic over sorts *) Some p ->
           search_unit_solution p
       | None -> raise Finished)
  
(* remove initial upper case letter *)
(* incomplete .R .S *)
let lower =
  function
...
let verbal = (* root verbal forms *)
  [ Root; Abso; (* abs-ya with aa preverb *) Pv; Pvk ]
and nominal = [ Noun; A; An; Ifc; Pron; Krid; Voca; Inv; Iic; Iik ]
and iic = [ Iic; Iik; A; An ]
and iiv = [ Iiv ]
      ...
      then "participial"
        ...
        then "iic"
          ...
          then "indeclinable"
            ...
            if entry = iiv
            then "iiv"
            else failwith "Unknown entry for stemmer"
  
let warn0 sort word =
  let message =
    "No " ^
      ((label sort) ^ (" solution for " ^ ((Canon.decode word) ^ "\n")))
  in output_string stdout message
and warn2 sort word =
  let message =
    "No unit " ^
      ((label sort) ^ (" solution for " ^ ((Canon.decode word) ^ "\n")))
...
let stem sort ident = (* phases in sort must be accepting *)
  let word = Encode.normalize (lower ident)
  in
    match segment1_initial sort word with
    | Some p ->
        (try search_unit_solution p
         with | Finished -> (warn2 sort word; "?")
         | Solution word -> Canon.decode_ref (List.rev word)
         | Solution2 (word1, word2) ->
             let s1 = Canon.decode_ref (Word.mirror word1)
             and s2 = Canon.decode_ref (Word.mirror word2)
             in s1 ^ ("2" ^ (s2 ^ "\n")))
    | None -> (warn0 sort word; "0")
  
(* necessary since iic is not accepting *)
let search_iic ident =
  let word = Encode.normalize (lower ident)
  in
...
let injunctions = (* NB: dhik treated otherwise now *)
  [ "aye"; "are"; "aho"; "aa.h"; "prasiida"; "svasti"; "bho"; "bho.h";
    "hanta"; "haa" ]
  
let nominal_stem case = stem nominal
and (* to do check case Noun+Ifc *) verbal_stem = stem verbal
and participial_stem case = stem participial
and (* to do check case *) iic_stem = search_iic
