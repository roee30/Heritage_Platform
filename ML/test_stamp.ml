...
let check_data_version () =
  let resources_version_file =
    Paths.skt_resources_dir ^ "DATA/version.rem" in
  let (data_format_version, data_version) =
    (Gen.gobble resources_version_file : (int * string))
  in
    if Control.data_format_version > data_format_version
    then
      (print_string "Your linguistic data are stale\n";
       print_string
         "Your must install a recent version of Heritage_Resources\n";
       exit 1)
    else
      if Control.data_format_version < data_format_version
      then
        ...
        if data_version = Version.version
        then ()
        else
          (print_string "Warning: this platform release ";
           ...
           print_string
             "while the currently installed Heritage_Resources at ";
